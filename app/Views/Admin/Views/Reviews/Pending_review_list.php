            ...
            <table class='table table-bordered table-striped' style='font-size:15px'>
                ...
                <?php if (isset($reviews[0])) {
                    foreach ($reviews as $row) { ?>
                        <tr>
                            <td width='150px'><?php if($row->first_name != ''){ echo $row->first_name.' '.$row->last_name;}else{ echo $row->user_id;} ?></td>
                            <td width='250px'><?php if($row->title != ''){?><a target='_blank' href='<?= base_url($row->category_name.'/'.$row->product_slug);?>'><?php echo $row->title;} ?></a></td>
