...
class Reviews extends BaseController
{
    ...
    {
        $data['reviews'] = $this->db->table('reviews')->select('*')->where('status', '0')->where('soft_delete', '0')->get()->getResult();
        // echo '<pre>';print_r($data);exit;
        return view('Admin/Views/Reviews/Pending_review_list', $data);
    }
    public function reviews_list()
    {
        $data['reviews'] = $this->db->table('reviews')->select('*')->where('status', '1')->where('soft_delete', '0')->get()->getResult();
        // echo '<pre>';print_r($data);exit;
        return view('Admin/Views/Reviews/reviews_list', $data);
    }
    public function deleted_list()
    {
        $data['reviews'] = $this->db->table('reviews')->select('*')->where('soft_delete', '1')->get()->getResult();
        // echo '<pre>';print_r($data);exit;
